        ...
        <div>
             <table>
                <tbody>
                 {vip_Results.map(vip => (
                     <tr key={vip.vip_id}>
                        <td>会员编号：{vip.vip_id}</td>
                        <td>会员姓名：{vip.vipname}</td>
...
export function AddPoints() {
    const [addPoints, setaddPoints] = useState(0); // 增加的积分
    ...
    function handleAddPointsClick() {
        const memberToUpdate = initialvip.filter(
            v => v.vipname === vipIdToAddPoints || v.phone === vipIdToAddPoints
        );
        ...
        if (memberToUpdate.length > 0) {
            for (const item of initialvip) {
                ...
                if (member) {                   
                    const addVipdata = { ...item, nowpoints: item.nowpoints + addPoints };
                    changeVIP(item.vip_id, addPoints);
                    setVip_AddResults((prestate) => [addVipdata]);
                    break;
                }
            }
            alert('积分添加成功');
    ...
    function handleDeletePointsClick() {
        ...
        if (memberToUpdate.length > 0) {
            for (const item of initialvip) {
                const member = memberToUpdate.find(member => member.vip_id === item.vip_id);
                if (member) {                   
                    const deleVipdata = { ...item, nowpoints: item.nowpoints -deletePoints };
                    changeVIP(item.vip_id, -deletePoints);
                    setVip_DeleResults((prestate) => [deleVipdata]);
                    break;
                }
            }
            alert('积分删除成功');
    ...
    return (
        <>
            <div>
                ...
                <input type="number" value={deletePoints}
                onChange={e => {
                            const num=Number(e.target.value);
                            if(num>0)
                            {

...
func (s *StorageService) UpdatekucunNumServ(ctx context.Context, tablename string, value float64, keyword string, id string, action string) error {
	err := s.storagerepo.UpdateInventoryQuantity(ctx, tablename, value, keyword, id, action)
	if err != nil {
		fmt.Println("service", err)
		return err
	}
